    ...
    <div className="relative w-full overflow-hidden px-4 py-24 border-t border-border/50 bg-card/20">
      <div className="relative mx-auto flex max-w-4xl flex-col items-center text-center mb-16">
        ...
        <p className="text-muted-foreground max-w-2xl">
          Choose the plan that fits your career goals. Whether you're a student
          or a senior executive, we have a plan for you.
        </p>
      </div>
        ...
        <PricingTableBody>
          {RESUME_FEATURES.map((feature) => (
            <PricingTableRow key={feature.label}>
              <PricingTableHead className="font-medium text-muted-foreground">
                {feature.label}
              </PricingTableHead>
              {feature.values.map((value) => (
                <PricingTableCell
                  key={`${feature.label}-${String(value)}`}
